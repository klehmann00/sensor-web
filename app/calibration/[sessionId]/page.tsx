...
// === TRANSFORMATION LEARNING INTERFACES ===
interface TransformationLearningState {
  state: 'idle' | 'learning' | 'locked';
  startIndex: number;
  endIndex: number;
}

interface TransformationResult {
  forwardAxis: 'x' | 'y' | 'z' | null;
  forwardAxisSign: 1 | -1;
  confidence: number;
  samplesUsed: number;
  correlationScores: { x: number; y: number; z: number };
  states: TransformationLearningState[];
}
...
interface CalibrationResult {
  ...
  accelLinearXFiltered: number[];
  accelLinearYFiltered: number[];
  accelLinearZFiltered: number[];
...
function applyFloatingCalibration(
  ...
  const result: CalibrationResult = {
    ...
    actualSampleRate: 60,
    // Cross-verification trifecta
    accelLinearXFiltered: [],
    accelLinearYFiltered: [],
    accelLinearZFiltered: [],
  ...
  for (let i = 0; i < accelData.length; i++) {
    ...
    // Periodic logging - COMMENTED OUT TO REDUCE CONSOLE SPAM
    ...
    //   console.log('  accelLinearYFiltered = filteredLinearAccel.y =', filteredLinearAccel.y.toFixed(3), '✓ (now filtering linear, not raw!)');
    //   console.log('  observerAccelLinearY_fromGyro = speed * filteredGyroZ =', currentSpeed.toFixed(2), '*', filteredGyroZ.toFixed(4), '=', (currentSpeed * filteredGyroZ).toFixed(3));
    //   console.log('  observerAccelLinearY_fromMag = speed * magHeadingRate =', currentSpeed.toFixed(2), '*', magHeadingRate.toFixed(4), '=', (currentSpeed * magHeadingRate).toFixed(3));
    //   console.log('');
    //   console.log('  gyroFilteredZ_observed = filteredGyroZ =', filteredGyroZ.toFixed(4), '✓');
    //   console.log('  observerGyroZ_fromAccel = filteredLinearAccel.y / speed =', filteredLinearAccel.y.toFixed(3), '/', currentSpeed.toFixed(2), '=',
    //     currentSpeed > 1 ? (filteredLinearAccel.y / currentSpeed).toFixed(4) : '0.0000 (speed too low)');
    //   console.log('  observerGyroZ_fromMag = magHeadingRate =', magHeadingRate.toFixed(4), '(from filtered mag heading change)');
    // }

    // === STORE ALL LINEAR ACCEL AXES (for transformation learning) ===
    const accelLinearXFiltered = filteredLinearAccel.x;
    const accelLinearYFiltered = filteredLinearAccel.y;
    const accelLinearZFiltered = filteredLinearAccel.z;

    result.accelLinearXFiltered.push(accelLinearXFiltered);
    result.accelLinearYFiltered.push(accelLinearYFiltered);
    result.accelLinearZFiltered.push(accelLinearZFiltered);

    // === LATERAL ACCELERATION TRIFECTA (observers use RAW data) ===
    // 2. Predicted from RAW gyroscope (centripetal acceleration: a = v*ω)
    const observerAccelLinearY_fromGyro = currentSpeed * gyro.z;

    // 3. Predicted from RAW magnetometer (heading change → rotation rate → centripetal accel)
    // Calculate raw heading rate from unfiltered magnetometer (rawMagHeading already calculated at line 280)
    const rawMagHeadingRate = i > 0 ? (rawMagHeading - (result.heading_observed[i-1] || rawMagHeading)) / deltaTime : 0;
    const observerAccelLinearY_fromMag = currentSpeed * rawMagHeadingRate;
    ...
    // === ROTATION RATE TRIFECTA (observers use RAW or FILTERED data) ===
    // 1. Measured from FILTERED gyroscope (this is the "real" signal we're comparing against)
    const gyroFilteredZ_observed = filteredGyroZ;

    // 2. Predicted from RAW accelerometer (ω = a/v, from centripetal acceleration)
    const observerGyroZ_fromAccel = currentSpeed > 1 ? (linearAccel.y / currentSpeed) : 0;

    // 3. Predicted from RAW magnetometer (heading change rate)
    const observerGyroZ_fromMag = rawMagHeadingRate;
  ...
  //   accelLinearYFiltered: result.accelLinearYFiltered?.length || 0,
  //   observerAccelLinearY_fromGyro: result.observerAccelLinearY_fromGyro?.length || 0,
  //   observerAccelLinearY_fromMag: result.observerAccelLinearY_fromMag?.length || 0,
  //   gyroFilteredZ_observed: result.gyroFilteredZ_observed?.length || 0,
  //   observerGyroZ_fromAccel: result.observerGyroZ_fromAccel?.length || 0,
  //   observerGyroZ_fromMag: result.observerGyroZ_fromMag?.length || 0,
  //   heading_observed: result.heading_observed?.length || 0,
  //   observerHeading_fromAccel: result.observerHeading_fromAccel?.length || 0,
  //   observerHeading_fromGyro: result.observerHeading_fromGyro?.length || 0
  // });

  // Sample values from turn section
  // if (result.accelLinearYFiltered && result.accelLinearYFiltered.length > 2800) {
  //   console.log('Turn section (2800-2810) values:');
  //   console.log('  accelY_real:', result.accelLinearYFiltered.slice(2800, 2810).map(v => v.toFixed(2)));
  //   console.log('  accelY_gyro:', result.observerAccelLinearY_fromGyro.slice(2800, 2810).map(v => v.toFixed(2)));
  //   console.log('  accelY_mag:', result.observerAccelLinearY_fromMag.slice(2800, 2810).map(v => v.toFixed(2)));
  // }

  // === CHECKPOINT: Just before transformation learning ===
  console.log("=== TRANSFORMATION LEARNING DEBUG: REACHED LINE 500 ===");
  console.log("DEBUG: If you see this message, the function execution reached this point!");

  // === RUN TRANSFORMATION LEARNING ===
  console.log(">>> ABOUT TO RUN TRANSFORMATION LEARNING");
  console.log('>>> Transformation input check:', {
    hasX: !!result.accelLinearXFiltered,
    hasY: !!result.accelLinearYFiltered,
    hasZ: !!result.accelLinearZFiltered,
    hasGPS: !!result.virtualForwardAccel,
    lengthX: result.accelLinearXFiltered?.length || 0,
    lengthY: result.accelLinearYFiltered?.length || 0,
    lengthZ: result.accelLinearZFiltered?.length || 0,
    lengthGPS: result.virtualForwardAccel?.length || 0
  });
  result.transformationResult = learnTransformation(
    result.accelLinearXFiltered,
    result.accelLinearYFiltered,
    result.accelLinearZFiltered,
    result.virtualForwardAccel,
    0.2,  // Lower threshold: 0.2 m/s² (was 0.5)
    5     // Lower min duration: 5 samples (was 10)
  );
  console.log(">>> TRANSFORMATION COMPLETE:", result.transformationResult);

  return result;
}


// === TRANSFORMATION LEARNING FUNCTION ===
function learnTransformation(
  accelLinearX: number[],
  accelLinearY: number[],
  accelLinearZ: number[],
  gpsAccel: number[],
  accelThreshold: number = 0.5,
  minEventDuration: number = 10
  ...
  const result: TransformationResult = {
    ...
    confidence: 0,
    samplesUsed: 0,
    correlationScores: { x: 0, y: 0, z: 0 },
    states: []
  };

  // Input validation
  if (!accelLinearX || !accelLinearY || !accelLinearZ || !gpsAccel) {
    console.warn('⚠️ Transformation learning: undefined input arrays');
    return result;
  }

  if (accelLinearX.length === 0 || accelLinearY.length === 0 ||
      accelLinearZ.length === 0 || gpsAccel.length === 0) {
    console.warn('⚠️ Transformation learning: empty input arrays');
    return result;
  }

  // Use minimum length instead of requiring exact match
  ...
  if (accelLinearX.length !== gpsAccel.length) {
    console.warn('⚠️ Transformation learning: array length mismatch, using minimum', {
      accelX: accelLinearX.length,
      accelY: accelLinearY.length,
      accelZ: accelLinearZ.length,
      gpsAccel: gpsAccel.length,
      usingMinLength: minLength
    });
  }

  console.log('>>> Starting event detection...', { totalSamples: minLength, threshold: accelThreshold, minDuration: minEventDuration });

  const events: Array<{start: number, end: number, avgAccel: number}> = [];
  let eventStart = -1;

  for (let i = 0; i < minLength; i++) {
    const significant = Math.abs(gpsAccel[i]) > accelThreshold;
    
    if (significant && eventStart === -1) {
      eventStart = i;
    } else if (!significant && eventStart !== -1) {
      const duration = i - eventStart;
      if (duration >= minEventDuration) {
        const sum = gpsAccel.slice(eventStart, i).reduce((a, b) => a + b, 0);
        const avgAccel = sum / duration;
        events.push({ start: eventStart, end: i, avgAccel });
      }
      eventStart = -1;
    }
  }

  console.log('>>> Event detection complete:', {
    eventsFound: events.length,
    events: events.map(e => ({ start: e.start, end: e.end, duration: e.end - e.start, avgAccel: e.avgAccel.toFixed(3) }))
  });

  const correlations = { x: 0, y: 0, z: 0 };
  let totalSamples = 0;
  
  for (const event of events) {
    const { start, end, avgAccel } = event;
    const duration = end - start;
    
    const avgX = accelLinearX.slice(start, end).reduce((a, b) => a + b, 0) / duration;
    const avgY = accelLinearY.slice(start, end).reduce((a, b) => a + b, 0) / duration;
    const avgZ = accelLinearZ.slice(start, end).reduce((a, b) => a + b, 0) / duration;
    
    correlations.x += avgAccel * avgX * duration;
    correlations.y += avgAccel * avgY * duration;
    correlations.z += avgAccel * avgZ * duration;
  ...
  if (totalSamples > 0) {
    correlations.x /= totalSamples;
    correlations.y /= totalSamples;
    correlations.z /= totalSamples;
  }
  
  result.correlationScores = correlations;
  result.samplesUsed = totalSamples;
  
  const absCorr = {
    x: Math.abs(correlations.x),
    y: Math.abs(correlations.y),
    z: Math.abs(correlations.z)
  };
  
  let maxCorr = 0;
  let bestAxis: 'x' | 'y' | 'z' = 'x';
  
  if (absCorr.x > maxCorr) { maxCorr = absCorr.x; bestAxis = 'x'; }
  if (absCorr.y > maxCorr) { maxCorr = absCorr.y; bestAxis = 'y'; }
  if (absCorr.z > maxCorr) { maxCorr = absCorr.z; bestAxis = 'z'; }
  
  result.forwardAxis = bestAxis;
  result.forwardAxisSign = correlations[bestAxis] >= 0 ? 1 : -1;
  
  const minSamplesForConfidence = 100;
  const sampleConfidence = Math.min(1, totalSamples / minSamplesForConfidence);
  
  const secondBest = Math.max(
    bestAxis !== 'x' ? absCorr.x : 0,
    bestAxis !== 'y' ? absCorr.y : 0,
    bestAxis !== 'z' ? absCorr.z : 0
  );
  
  const separationConfidence = maxCorr > 0 ? 
    Math.min(1, (maxCorr - secondBest) / maxCorr) : 0;
  
  result.confidence = sampleConfidence * separationConfidence;
  
  if (result.confidence > 0.7 && events.length > 0) {
    const lastEvent = events[events.length - 1];
    result.states.push({
      state: 'locked',
      startIndex: lastEvent.end,
      endIndex: accelLinearX.length - 1
    });
  }
  
  console.log('>>> TRANSFORMATION LEARNING RESULT:', {
    forwardAxis: result.forwardAxis,
    sign: result.forwardAxisSign > 0 ? '+' : '-',
    confidence: (result.confidence * 100).toFixed(1) + '%',
    samplesUsed: result.samplesUsed,
    correlations: {
      x: result.correlationScores.x.toFixed(3),
      y: result.correlationScores.y.toFixed(3),
      z: result.correlationScores.z.toFixed(3)
    },
    learningPeriods: result.states.filter(s => s.state === 'learning').length
  });
...
export default function CalibrationAnalysisPage() {
  ...
  const [signalControls, setSignalControls] = useState<Record<string, { visible: boolean; offset: number; color: string; width?: number; yAxisID?: string; label?: string }>>({
    // === RAW SIGNALS ===
    accelRawX: { visible: false, offset: 0, color: 'rgba(239, 68, 68, 0.3)', width: 1, label: 'accelRawX' },
    accelRawY: { visible: false, offset: 0, color: 'rgba(245, 158, 11, 0.3)', width: 1, label: 'accelRawY' },
    accelRawZ: { visible: false, offset: 0, color: 'rgba(59, 130, 246, 0.3)', width: 1, label: 'accelRawZ' },
    gyroRawX: { visible: false, offset: 0, color: 'rgba(139, 92, 246, 0.3)', width: 1, label: 'gyroRawX' },
    gyroRawY: { visible: false, offset: 0, color: 'rgba(236, 72, 153, 0.3)', width: 1, label: 'gyroRawY' },
    gyroRawZ: { visible: false, offset: 0, color: 'rgba(6, 182, 212, 0.3)', width: 1, label: 'gyroRawZ' },
    magX: { visible: false, offset: 0, color: '#10b981', width: 1, label: 'magX' },
    magY: { visible: false, offset: 0, color: '#14b8a6', width: 1, label: 'magY' },
    magZ: { visible: false, offset: 0, color: '#22c55e', width: 1, label: 'magZ' },

    // === FILTERED SIGNALS ===
    accelFilteredX: { visible: false, offset: 0, color: '#ef4444', width: 2, label: 'accelFilteredX' },
    accelFilteredY: { visible: false, offset: 0, color: '#f97316', width: 2, label: 'accelFilteredY' },
    accelFilteredZ: { visible: false, offset: 0, color: '#3b82f6', width: 2, label: 'accelFilteredZ' },
    gyroFilteredX: { visible: false, offset: 0, color: '#8b5cf6', width: 2, label: 'gyroFilteredX' },
    gyroFilteredY: { visible: false, offset: 0, color: '#ec4899', width: 2, label: 'gyroFilteredY' },
    gyroFilteredZ: { visible: false, offset: 0, color: '#06b6d4', width: 2, label: 'gyroFilteredZ' },

    // === LINEAR SIGNALS ===
    accelLinearX: { visible: false, offset: 0, color: '#dc2626', width: 3, label: 'accelLinearX' },
    accelLinearY: { visible: true, offset: 0, color: '#f97316', width: 3, label: 'accelLinearY' },
    accelLinearZ: { visible: false, offset: 0, color: '#2563eb', width: 3, label: 'accelLinearZ' }
  });
  ...
  console.log(`  ✅ ${key} ADDED to chart`);
    ...
    console.log('Lateral Accel Trifecta:', {
      measured: {
        min: Math.min(...calibrationResult.accelLinearYFiltered),
        max: Math.max(...calibrationResult.accelLinearYFiltered),
        avg: calibrationResult.accelLinearYFiltered.reduce((a, b) => a + b, 0) / calibrationResult.accelLinearYFiltered.length
      },
      fromGyro: {
    ...
    // === ADD LINEAR ACCELERATION DATASETS TO CHART ===
    console.log('=== ADDING LINEAR ACCELERATION DATASETS TO CHART ===');
    console.log('Adding linear acceleration datasets:', {
      hasAccelXData: !!calibrationResult.accelLinearXFiltered,
      accelXLength: calibrationResult.accelLinearXFiltered?.length || 0,
      hasAccelYData: !!calibrationResult.accelLinearYFiltered,
      accelYLength: calibrationResult.accelLinearYFiltered?.length || 0,
      hasAccelZData: !!calibrationResult.accelLinearZFiltered,
      accelZLength: calibrationResult.accelLinearZFiltered?.length || 0,
      signalControlsExist: {
        accelLinearX: !!signalControls.accelLinearX,
        accelLinearY: !!signalControls.accelLinearY,
        accelLinearZ: !!signalControls.accelLinearZ
      }
    });

    // Add linear acceleration signals (gravity removed, filtered)
    if (calibrationResult.accelLinearXFiltered) {
      addDataset('accelLinearX', calibrationResult.accelLinearXFiltered, signalControls.accelLinearX);
    }
    if (calibrationResult.accelLinearYFiltered) {
      addDataset('accelLinearY', calibrationResult.accelLinearYFiltered, signalControls.accelLinearY);
    }
    if (calibrationResult.accelLinearZFiltered) {
      addDataset('accelLinearZ', calibrationResult.accelLinearZFiltered, signalControls.accelLinearZ);
    }
        ...
        {isLoading ? (
          ...
          <>
            ...
            {accelerationAnalysisData && (
              <div className="bg-white rounded-lg shadow-lg p-2 mb-2">
                <h2 className="text-base font-bold mb-1 text-gray-800">
                  🔬 Acceleration Analysis: Raw → Linear → GPS Comparison
                </h2>
                <p className="text-xs text-gray-600 mb-2">
                  <strong>THICK colored lines</strong> = Transformed vehicle coordinates (filtered α={orientationFilterAlpha.toFixed(3)}).
                  🟢 GPS ground truth | 🔴 Red x&apos; (accel/brake) | 🔵 Blue y&apos; (turn) | 🟩 Green z&apos; (bump).
                ...
                <div className="mt-2 text-xs text-gray-600">
                  <strong>✅ Orientation Test:</strong> During constant-speed cruise, x&apos;/y&apos;/z&apos; should be near zero.
                  If x&apos; matches GPS spikes (accel/brake) and y&apos; shows turns → orientation is CORRECT!
              ...
              <div className="bg-white rounded-lg shadow-lg p-2 mb-2 border-4 border-blue-300">
                <div className="flex items-center gap-2 mb-2">
                  <h2 className="text-2xl font-bold text-gray-800">
                    📍 Vehicle Reference Frame {viewMode === 'scrollable' && `(showing ${scrollPosition} to ${scrollPosition + windowSize})`}
                  </h2>
                ...
                <p className="text-xs text-gray-600 mb-2">
                  Transformed vehicle coordinates (x′, y′, z′) with zero baseline - this is what actually matters!
                </p>

                {avgConfidence > 0.8 && (
                  <div className="mb-3 p-2 bg-green-100 border border-green-400 rounded-lg">
                    <div className="text-green-800 text-sm font-semibold">
                      ✓ Reference frame stabilized - vehicle coordinates established
                    </div>
                  </div>
                )}
                        ...
                        tooltip: {
                          callbacks: {
                            label: (context) => {
                              ...
                              return `${context.dataset.label}: ${value !== null ? value.toFixed(2) : '0.00'} m/s²`;
                            }
                          }
                        }
                      }
                ...
                <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                  <h3 className="font-bold text-gray-800 text-sm mb-2">✓ Calibration Success Indicators:</h3>
                  <ul className="list-disc list-inside space-y-1 text-xs text-gray-700">
                    <li>
                      <strong className="text-red-600">x′</strong> oscillates around zero = forward/back acceleration only
                    </li>
                    <li>
                      <strong className="text-blue-600">y′</strong> oscillates around zero = lateral (turning) forces only
                    </li>
                    <li>
                      <strong className="text-green-600">z′</strong> oscillates around zero = vertical (bumps) forces only
                    </li>
                  ...
                  return (
                    <div className="text-sm">
                      <div className={`font-bold ${gravityPass ? 'text-green-600' : 'text-red-600'}`}>
                        {gravityPass ? '✅ GRAVITY FOUND' : '❌ GRAVITY NOT FOUND'}
                      </div>
                      <div className="text-xs mt-1">
                        Magnitude: {gravityMag.toFixed(2)} m/s²
                        (target: 9.80 m/s², error: {gravityError.toFixed(1)}%)
                      </div>
                      <div className="text-xs">
            ...
            {gravityMagnitudeData && (
              <div className="bg-white rounded-lg shadow-lg p-2 mb-2">
                <h2 className="text-base font-bold mb-1 text-gray-800">
                  📈 Step 1: Gravity Magnitude Convergence
                </h2>
                <p className="text-xs text-gray-600 mb-2">
                  Shows how gravity magnitude converges to 9.8 m/s² over time. Should stabilize within first 5 seconds (~300 samples).
                </p>
                ...
                <div className="mt-2 text-xs text-gray-600">
                  Green line = actual gravity magnitude | Red dashed line = target (9.8 m/s²)
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-lg shadow-lg p-2 mb-2">
                <div className="flex justify-between items-center mb-1">
                  <h2 className="text-xl font-bold text-gray-800">
                    📊 Calibration State Timeline {viewMode === 'scrollable' && `(showing ${scrollPosition} to ${scrollPosition + windowSize})`}
                  </h2>
                  <button
                    onClick={() => setCollapsedCharts(prev => ({ ...prev, stateTimeline: !prev.stateTimeline }))}
                    className="text-sm px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded"
                  >
                    {collapsedCharts.stateTimeline ? '▶ Show' : '▼ Hide'}
                  </button>
                </div>
            ...
            {virtualVsRealData && (
              <div className="bg-white rounded-lg shadow-lg p-2 mb-2">
                <div className="flex justify-between items-center mb-1">
                  <h2 className="text-xl font-bold text-gray-800">
                    🎯 Virtual vs Real Acceleration {viewMode === 'scrollable' && `(showing ${scrollPosition} to ${scrollPosition + windowSize})`}
                  </h2>
                  <button
                    onClick={() => setCollapsedCharts(prev => ({ ...prev, virtualVsReal: !prev.virtualVsReal }))}
                    className="text-sm px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded"
                  >
                    {collapsedCharts.virtualVsReal ? '▶ Show' : '▼ Hide'}
                  </button>
                </div>
            ...
            {confidenceChartData && (
              <div className="bg-white rounded-lg shadow-lg p-2 mb-2">
                <h2 className="text-base font-bold mb-1 text-gray-800">
                  📊 Calibration Progress & Vehicle Speed {viewMode === 'scrollable' && `(showing ${scrollPosition} to ${scrollPosition + windowSize})`}
                </h2>
                <p className="text-xs text-gray-600 mb-2">
            ...
            {gravityForwardChartData && calibrationResult && (
              <div className="bg-white rounded-lg shadow-lg p-2 mb-2">
                <h2 className="text-base font-bold mb-1 text-gray-800">
                  📱 Raw Sensor Vectors (Phone Coordinates) {viewMode === 'scrollable' && `(showing ${scrollPosition} to ${scrollPosition + windowSize})`}
                </h2>
                <p className="text-xs text-gray-600 mb-2">
                ...
                <div className="mt-3 text-xs text-gray-600">
                  {(() => {
                    ...
                    return (
                      <span>
                        Gravity Magnitude: <strong>{gravityMag.toFixed(2)} m/s²</strong> (target: 9.8 m/s²)
                      </span>
                    );
                  })()}
                </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-4 mt-4">
              ...
              <ul className="list-disc list-inside space-y-1 text-xs text-gray-700">
                <li>
                  <strong>Gyro-Gated Learning:</strong> Forward vector only updates during straight-line motion (gyro &lt; 1.5 rad/s / ~86°/s)
                </li>
                <li>
                ...
                <li>
                  <strong>Forward Detection:</strong> Only learns from meaningful acceleration events (&gt; 0.5 m/s²) during straight driving
                </li>
                <li>
                  <strong>4-Factor Confidence:</strong> Gravity quality + Forward quality + Gyro stability + Forward update count (≥20)
                </li>
                <li>
